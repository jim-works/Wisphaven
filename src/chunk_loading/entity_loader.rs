...
pub fn do_loading(mut commands: Commands, mut level: ResMut<Level>, mut despawn_writer: EventWriter<DespawnChunkEvent>, 
    ...
    for (transform, loader) in loader_query.iter() {
        ...
        for i in 1..loader.lod_levels as usize {
            ...
            load_lod(i, &mut commands, &mut level, &transform, &loader, &mut loaded_lod);
...
fn load_lod(lod_level: usize, commands: &mut Commands, level: &mut ResMut<Level>, transform: &GlobalTransform, loader: &ChunkLoader, loaded_list: &mut HashSet<ChunkCoord>) {
