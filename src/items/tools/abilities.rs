...
use crate::{world::{events::{BlockDamageSetEvent, BlockHitEvent}, Level, BlockId, BlockCoord, LevelSystemSet}, util::Direction};

use super::{ToolResistance, calc_block_damage, Tool};
...
impl Plugin for ToolAbilitiesPlugin {
    fn build(&self, app: &mut App) {
        app.register_type::<AxeAbility>()
            ...
            .add_systems(Update, (axe_ability_system, shovel_ability_system).in_set(LevelSystemSet::Main));
    }
}
        ...
        if let Some(item) = item {
            if let Ok((tool, AxeAbility { max_blocks, search_radius, damage_mult })) = axe_ability_query.get(*item) {
                if let Some(block) = level.get_block_entity(*block_position) {
                    ...
                    let damage = calc_block_damage(resistance_query.get(block).copied().unwrap_or_default(), *tool);
                    //trigger ability
                    do_axe_ability(&level, damage*damage_mult, *block_position, *max_blocks, *search_radius, &id_query, &target_query, *item, &mut HashSet::new(), &mut commands, &mut writer);
                }
            }
        }
    }
                ...
                for z in -square_radius..square_radius+1 {
                    ...
                    if let Some(block) = level.get_block_entity(pos) {
                        if target_query.contains(block) {
                            if hits.insert(pos) {
                                if pos != initial_pos {
                                    level.damage_block(pos, damage, Some(tool), &id_query, writer, commands);
                                }
                                do_axe_ability(level, damage, pos, max_blocks, search_radius, id_query, target_query, tool, hits, commands, writer);
                            }
                ...
                for z in -square_radius..square_radius+1 {
                    ...
                    if let Some(block) = level.get_block_entity(pos) {
                        if target_query.contains(block) {
                            if hits.insert(pos) {
                                if pos != initial_pos {
                                    level.damage_block(pos, damage, None, &id_query, writer, commands);
                                }
                                do_axe_ability(level, damage, pos, max_blocks, search_radius, id_query, target_query, tool, hits, commands, writer);
                            }
                            if hits.len() >= max_blocks {
                ...
                for len in 0..(*length as i32) {
                    direction.for_each_in_plane(*radius as i32, |offset| {
                        ...
                        if let Some(block) = level.get_block_entity(coord) {
                            ...
                            let damage = calc_block_damage(resistance_query.get(block).copied().unwrap_or_default(), *tool)*damage_mult;
                            level.damage_block(coord, damage, Some(*item), &id_query, &mut writer, &mut commands);
                        }
                    });
                }
