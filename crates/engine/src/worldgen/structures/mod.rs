...
pub trait StructureGenerator {
    ...
    fn generate(&self, buffer: &mut BlockBuffer<BlockId>, world_seed: u64, world_pos: BlockCoord, local_pos: ChunkIdx, chunk: &GeneratingChunk) -> bool;
}
...
pub fn gen_structures(chunk: &mut GeneratingChunk, seed: u64, biomes: ColumnBiomes<CHUNK_SIZE>, biome_map: &UsedBiomeMap) -> BlockBuffer<BlockId> {
    ...
    if let Some(gen) = &biome.fallback_generator {
        gen.generate(&mut buf, seed, chunk.position.into(), ChunkIdx::new(0,0,0), chunk);
    }
