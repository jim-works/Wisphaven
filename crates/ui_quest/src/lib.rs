...
//         app.add_systems(Update, (spawn_ui,).chain().in_set(LevelSystemSet::Main))
...
// fn spawn_ui(mut commands: Commands, ui_query: Query<(), With<QuestUI>>) {
...
//     recipe_row_query: Query<(Entity, &QuestRow)>,
//     ui_root_query: Query<Entity, With<QuestUI>>,
...
//         recipe_row_query
//             .iter()
//             .map(|(entity, recipe)| (recipe.quest, entity)),
//     );
...
//             let row = world.get::<QuestRow>(entity).cloned().unwrap();
//             let cached_recipe = world.get::<CachedEntityRecipe>(row.quest).cloned().unwrap();
...
//                                 ..default()
//                             },
//                             QuestAcceptButton { quest: row.quest },
//                             PickingBehavior {
//                                 // want to be able to scroll the background
//                                 should_block_lower: false,
//                                 is_hoverable: true,
...
//                              button_query: Query<&QuestAcceptButton>,
...
//                                         if let Ok(recipe) = recipe_query.get(button.quest) {
